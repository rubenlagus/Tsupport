...
public class ConnectionsManager implements Action.ActionDelegate, TcpConnection.TcpConnectionDelegate {
        ...
        public void run() {
            ...
            if (datacenters != null) {
                ...
                if (sendingPushPing && lastPushPingTime < System.currentTimeMillis() - 30000 || Math.abs(lastPushPingTime - System.currentTimeMillis()) > 60000 * 3 + 10000) {
                    ...
                    FileLog.e("tsupport", "push ping timeout");
                }
                if (lastPushPingTime < System.currentTimeMillis() - 60000 * 3) {
                    FileLog.e("tsupport", "time for push ping");
            ...
            if (lastPauseTime != 0 && lastPauseTime < currentTime - nextSleepTimeout) {
                ...
                if (!dontSleep) {
                    if (!paused) {
                        FileLog.e("tsupport", "pausing network and timers by sleep time = " + nextSleepTimeout);
                    ...
                    FileLog.e("tsupport", "don't sleep 30 seconds because of salt, upload or download request");
                }
            }
            if (paused) {
                paused = false;
                FileLog.e("tsupport", "resume network and timers");
            }
    ...
    private void resumeNetworkInternal() {
        if (paused) {
            ...
            FileLog.e("tsupport", "wakeup network in background");
        } else if (lastPauseTime != 0) {
            lastPauseTime = System.currentTimeMillis();
            FileLog.e("tsupport", "reset sleep timeout");
        }
    }
    ...
    public void applicationMovedToForeground() {
        ...
        Utilities.stageQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                if (paused) {
                    nextSleepTimeout = 30000;
                    FileLog.e("tsupport", "reset timers by application moved to foreground");
                }
            }
        });
    }
    ...
    public void setAppPaused(final boolean value, final boolean byScreenState) {
        Utilities.stageQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                if (!byScreenState) {
                    appPaused = value;
                    FileLog.e("tsupport", "app paused = " + value);
                }
                if (value) {
                    if (byScreenState) {
                        if (lastPauseTime == 0) {
                    ...
                    FileLog.e("tsupport", "reset app pause time");
    ...
    public long performRpc(final TLObject rpc, final RPCRequest.RPCRequestDelegate completionBlock, final RPCRequest.RPCQuickAckDelegate quickAckBlock, final boolean requiresCompletion, final int requestClass, final int datacenterId, final boolean runQueue) {
        if (rpc == null || !UserConfig.isClientActivated() && (requestClass & RPCRequest.RPCRequestClassWithoutLogin) == 0) {
            FileLog.e("tsupport", "can't do request without login " + rpc);
            return 0;
        }
            ...
            public void run() {
                ...
                for (int i = 0; i < requestQueue.size(); i++) {
                    ...
                    if (request.token == token) {
                        ...
                        FileLog.d("tsupport", "===== Cancelled queued rpc request " + request.rawRequest);
                ...
                if (!ifNotSent) {
                    for (int i = 0; i < runningRequests.size(); i++) {
                        ...
                        if (request.token == token) {
                            ...
                            FileLog.d("tsupport", "===== Cancelled running rpc request " + request.rawRequest);
                    ...
                    if (!found) {
                        FileLog.d("tsupport", "***** Warning: cancelling unknown request");
                    }
                }
            }
        });
    ...
    private void processRequestQueue(int requestClass, int _datacenterId) {
        ...
        for (int i = 0; i < runningRequests.size(); i++) {
            ...
            if (datacenters.size() > 1 && (request.flags & RPCRequest.RPCRequestClassTryDifferentDc) != 0) {
                ...
                if (requestStartTime != 0 && requestStartTime < currentTime - timeout) {
                    FileLog.e("tsupport", "move " + request.rawRequest + " to requestQueue");
                    requestQueue.add(request);
                    runningRequests.remove(i);
                    i--;
                    continue;
            ...
            if (((Math.abs(currentTime - request.runningStartTime) > maxTimeout) && (currentTime > request.runningMinStartTime || Math.abs(currentTime - request.runningMinStartTime) > 60.0)) || forceThisRequest) {
                if (!forceThisRequest && request.transportChannelToken > 0) {
                    if ((request.flags & RPCRequest.RPCRequestClassGeneric) != 0 && request.transportChannelToken == connection.channelToken) {
                        FileLog.d("tsupport", "Request token is valid, not retrying " + request.rawRequest);
                        continue;
                    } else {
                        if (connection.channelToken != 0 && request.transportChannelToken == connection.channelToken) {
                            FileLog.d("tsupport", "Request download token is valid, not retrying " + request.rawRequest);
                            continue;
                        }
                    }
                }
        ...
        for (int i = 0; i < requestQueue.size(); i++) {
            ...
            if (requestLength != 0) {
                ...
                FileLog.e("tsupport", "***** Couldn't serialize " + request.rawRequest);
            }
    ...
    TLRPC.TL_protoMessage wrapMessage(TLObject message, TcpConnection connection, boolean meaningful) {
        ...
        if (os.length() != 0) {
            ...
            FileLog.e("tsupport", "***** Couldn't serialize " + message);
            return null;
        }
    }
        ...
        for (int a = 0; a < messagesToSend.size(); a++) {
            ...
            if (currentSize >= 3 * 1024 || a == messagesToSend.size() - 1) {
                ...
                if (transportData != null) {
                    ...
                    FileLog.e("tsupport", "***** Transport data is nil");
                }
        ...
        if (messages.size() == 1) {
            ...
            if (BuildVars.DEBUG_VERSION) {
                if (message.body instanceof TLRPC.invokeWithLayer18) {
                    FileLog.d("tsupport", connection.getSissionId() + ":DC" + datacenter.datacenterId + "> Send message (" + message.seqno + ", " + message.msg_id + "): " + ((TLRPC.invokeWithLayer18)message.body).query);
                    ...
                    if (r.query instanceof TLRPC.invokeWithLayer18) {
                        FileLog.d("tsupport", connection.getSissionId() + ":DC" + datacenter.datacenterId + "> Send message (" + message.seqno + ", " + message.msg_id + "): " + ((TLRPC.invokeWithLayer18)r.query).query);
                    } else {
                        FileLog.d("tsupport", connection.getSissionId() + ":DC" + datacenter.datacenterId + "> Send message (" + message.seqno + ", " + message.msg_id + "): " + r.query);
                    }
                } else {
                    FileLog.d("tsupport", connection.getSissionId() + ":DC" + datacenter.datacenterId + "> Send message (" + message.seqno + ", " + message.msg_id + "): " + message.body);
                }
            }
            ...
            if (msg_time < currentTime - 30000 || msg_time > currentTime + 25000) {
                FileLog.d("tsupport", "wrap in messages continaer");
            ...
            ConnectionsManager.getInstance().performRpc(req, new RPCRequest.RPCRequestDelegate() {
                ...
                public void run(TLObject response, TLRPC.TL_error error) {
                    if (error == null) {
                        ...
                        UserConfig.saveConfig(false);
                        saveSession();
                        FileLog.e("tsupport", "registered for internal push");
                    } else {
                        UserConfig.registeredForInternalPush = false;
                    }
    ...
    void processMessage(TLObject message, long messageId, int messageSeqNo, long messageSalt, TcpConnection connection, long innerMsgId, long containerMessageId) {
        if (message == null) {
            FileLog.e("tsupport", "message is null");
            return;
        }
        Datacenter datacenter = datacenterWithId(connection.getDatacenterId());
        ...
        if (message instanceof TLRPC.TL_new_session_created) {
            ...
            if (!connection.isSessionProcessed(newSession.unique_id)) {
                FileLog.d("tsupport", "New session:");
                FileLog.d("tsupport", String.format("    first message id: %d", newSession.first_msg_id));
                FileLog.d("tsupport", String.format("    server salt: %d", newSession.server_salt));
                FileLog.d("tsupport", String.format("    unique id: %d", newSession.unique_id));
            ...
            if ((connection.transportRequestClass & RPCRequest.RPCRequestClassPush) == 0) {
                ...
                FileLog.e("tsupport", "received push ping");
                sendingPushPing = false;
            }
            ...
            for (long session : lst) {
                if (session == res.session_id) {
                    ...
                    FileLog.d("tsupport", String.format("Destroyed session %d (%s)", res.session_id, res instanceof TLRPC.TL_destroy_session_ok ? "ok" : "not found"));
                    break;
                }
            }
            ...
            FileLog.d("tsupport", "object in rpc_result is " + resultContainer.result);
            if (resultContainer.result instanceof TLRPC.RpcError) {
                ...
                FileLog.e("tsupport", String.format("***** RPC error %d: %s", ((TLRPC.RpcError)resultContainer.result).error_code, errorMessage));
            ...
            if (!ignoreResult) {
                            ...
                            if (resultContainer.result instanceof TLRPC.RpcError) {
                                ...
                                if (request.rawRequest == null || !request.rawRequest.responseClass().isAssignableFrom(resultContainer.result.getClass())) {
                                    if (request.rawRequest == null) {
                                        FileLog.e("tsupport", "rawRequest is null");
                                    } else {
                                        FileLog.e("tsupport", "***** RPC error: invalid response class " + resultContainer.result + " (" + request.rawRequest.responseClass() + " expected)");
                                    }
                                    implicitError = new TLRPC.TL_error();
                                    implicitError.code = -1000;
                                    implicitError.text = "";
                        ...
                        if (!discardResponse) {
                            if (request.initRequest && !isError) {
                                if (datacenter.lastInitVersion != currentAppVersion) {
                                    ...
                                    saveSession();
                                    FileLog.e("tsupport", "init connection completed");
                                } else {
                                    FileLog.e("tsupport", "rpc is init, but init connection already completed");
                                }
                            }
                            request.completed = true;
                ...
                if (!found) {
                    FileLog.d("tsupport", "Response received, but request wasn't found.");
                    rpcCompleted(resultMid);
                }
            ...
            FileLog.e("tsupport", String.format("***** Bad message: %d", badMsgNotification.error_code));
            ...
            if ((connection.transportRequestClass & RPCRequest.RPCRequestClassPush) != 0) {
                FileLog.e("tsupport", "received internal push");
                if (paused) {
            ...
            FileLog.e("tsupport", "***** Error: unknown message class " + message);
        }
    }
    ...
    void generatePing(Datacenter datacenter, boolean push) {
        ...
        if (connection != null && (push || !push && connection.channelToken != 0)) {
            ...
            if (transportData != null) {
                if (push) {
                    FileLog.e("tsupport", "send push ping");
    ...
    public void tcpConnectionClosed(TcpConnection connection) {
        if (connection.getDatacenterId() == currentDatacenterId && (connection.transportRequestClass & RPCRequest.RPCRequestClassGeneric) != 0) {
            ...
            if (BuildVars.DEBUG_VERSION) {
                try {
                    ...
                    for (int a = 0; a < 2; a++) {
                        ...
                        FileLog.e("tsupport", "Network: " + info.getTypeName() + " status: " + info.getState() + " info: " + info.getExtraInfo() + " object: " + info.getDetailedState() + " other: " + info);
                    }
                    if (networkInfos.length == 0) {
                        FileLog.e("tsupport", "no network available");
                    }
                } catch (Exception e) {
                    FileLog.e("tsupport", "NETWORK STATE GET ERROR", e);
                }
            }
            ...
            FileLog.e("tsupport", "call connection closed");
            sendingPushPing = false;
    ...
    public void tcpConnectionReceivedData(TcpConnection connection, ByteBufferDesc data, int length) {
        ...
        if (length == 4) {
            ...
            FileLog.e("tsupport", "mtproto error = " + error);
        ...
        if (keyId == 0) {
            ...
            if (datacenter.authKeyId == 0 || keyId != datacenter.authKeyId) {
                FileLog.e("tsupport", "Error: invalid auth key id " + connection);
                connection.suspendConnection(true);
                connection.connect();
                return;
            }
            ...
            if (messageSessionId != connection.getSissionId()) {
                FileLog.e("tsupport", String.format("***** Error: invalid message session ID (%d instead of %d)", messageSessionId, connection.getSissionId()));
                finishUpdatingState(connection);
                return;
            }
            ...
            if (!Utilities.arraysEquals(messageKey, 0, realMessageKeyFull, realMessageKeyFull.length - 16)) {
                FileLog.e("tsupport", "***** Error: invalid message key");
                connection.suspendConnection(true);
                connection.connect();
                return;
            }
            ...
            if (!doNotProcess) {
                ...
                if (message == null) {
                    FileLog.e("tsupport", "***** Error parsing message: " + constructor);
                } else {
                    FileLog.e("tsupport", "received object " + message);
