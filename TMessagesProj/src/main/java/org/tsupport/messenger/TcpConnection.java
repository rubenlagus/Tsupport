...
public class TcpConnection extends ConnectionContext {
        ...
        selector.scheduleTask(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    FileLog.d("tsupport", String.format(TcpConnection.this + " Connecting (%s:%d)", hostAddress, hostPort));
                    firstPacket = true;
    ...
    private void handleConnectionError(Exception e) {
        ...
        FileLog.d("tsupport", "Reconnect " + hostAddress + ":" + hostPort + " " + TcpConnection.this);
        try {
    ...
    private void suspendConnectionInternal() {
        ...
        FileLog.d("tsupport", "suspend connnection " + TcpConnection.this);
    ...
    private void readData(ByteBuffer buffer) throws Exception {
        ...
        if (restOfTheData != null) {
            if (lastPacketLength == 0) {
                //FileLog.e("tsupport", this +  " write addition data to restOfTheData");
                if (restOfTheData.capacity() - restOfTheData.position() >= buffer.limit()) {
                    ...
                    //FileLog.e("tsupport", this +  " no need to recreate buffer");
                } else {
                    ...
                    //FileLog.e("tsupport", this +  " NEED to recreate buffer");
                }
            } else {
                //FileLog.e("tsupport", this +  " write buffer to restOfTheData buffer of len = " + lastPacketLength);
                int len = 0;
                if (lastPacketLength - restOfTheData.position() <= buffer.limit()) {
                    len = lastPacketLength - restOfTheData.position();
                    //FileLog.e("tsupport", this +  " received buffer - OK!");
                } else {
                    len = buffer.limit();
                    //FileLog.e("tsupport", this +  " received buffer less than need");
                }
                ...
                if (restOfTheData.position() != lastPacketLength) {
                    //FileLog.e("tsupport", this +  " don't get much data to restOfTheData");
                    return;
                } else {
                    //FileLog.e("tsupport", this +  " get much data to restOfTheData - OK!");
                    if (buffer.hasRemaining()) {
                        parseLaterBuffer = buffer;
                        //FileLog.e("tsupport", this +  " something remain in the received buffer");
                    } else {
        ...
        while (buffer.hasRemaining()) {
            ...
            if ((fByte & (1 << 7)) != 0) {
                ...
                if (buffer.remaining() < 4) {
                    ...
                    lastPacketLength = 0;
                    //FileLog.e("tsupport", this +  " 1 - size less than 4 bytes - write to free buffer");
                    if (reuseLater != null) {
                        BuffersStorage.getInstance().reuseFreeBuffer(reuseLater);
                        //FileLog.e("tsupport", this +  " 1 - reuse later buffer1");
                    }
                    break;
                }
            ...
            if (fByte != 0x7f) {
                ...
                buffer.reset();
                if (buffer.remaining() < 4) {
                    //FileLog.e("tsupport", this +  " 2 - size less than 4 bytes - write to free buffer");
                    if (restOfTheData == null || restOfTheData != null && restOfTheData.position() != 0) {
                        ...
                        if (reuseLater != null) {
                            BuffersStorage.getInstance().reuseFreeBuffer(reuseLater);
                            //FileLog.e("tsupport", this +  " 2 - reuse later buffer1");
                        }
                    } else {
            ...
            if (currentPacketLength % 4 != 0 || currentPacketLength > 2 * 1024 * 1024) {
                FileLog.e("tsupport", "Invalid packet length");
                reconnect();
                return;
            }

            if (currentPacketLength < buffer.remaining()) {
                FileLog.d("tsupport", TcpConnection.this + " Received message len " + currentPacketLength + " but packet larger " + buffer.remaining());
            } else if (currentPacketLength == buffer.remaining()) {
                FileLog.d("tsupport", TcpConnection.this + " Received message len " + currentPacketLength + " equal to packet size");
            } else {
                FileLog.d("tsupport", TcpConnection.this + " Received packet size less(" + buffer.remaining() + ") then message size(" + currentPacketLength + ")");
                ...
                if (restOfTheData != null && restOfTheData.capacity() < len) {
                    ...
                    restOfTheData = null;
                    //FileLog.e("tsupport", this +  " not enough space for len, recreate buffer = " + len);
                }
                if (restOfTheData == null) {
                    //FileLog.e("tsupport", this +  " write to restOfTheData, get buffer len = " + len);
                    buffer.reset();
                ...
                if (reuseLater != null) {
                    BuffersStorage.getInstance().reuseFreeBuffer(reuseLater);
                    //FileLog.e("tsupport", this +  " 3 - reuse later buffer1");
                }
                return;
            }
            ...
            if (restOfTheData != null) {
                if (lastPacketLength != 0 && restOfTheData.position() == lastPacketLength || lastPacketLength == 0 && !restOfTheData.hasRemaining()) {
                    BuffersStorage.getInstance().reuseFreeBuffer(restOfTheData);
                    restOfTheData = null;
                    //FileLog.e("tsupport", this +  " restOfTheData parsed null it");
                } else {
                    restOfTheData.compact();
                    restOfTheData.limit(restOfTheData.position());
                    restOfTheData.position(0);
                    //FileLog.e("tsupport", this +  " restOfTheData NOT parsed, compact");
                }
            }

            if (parseLaterBuffer != null) {
                //FileLog.e("tsupport", this +  " there is parseLaterBuffer");
    ...
    public void handleDisconnect(PyroClient client, Exception e, boolean timedout) {
        ...
        if (e != null) {
            FileLog.d("tsupport", "Disconnected " + TcpConnection.this + " with error " + e);
        } else {
            FileLog.d("tsupport", "Disconnected " + TcpConnection.this);
        }
    ...
    public void connectedClient(PyroClient client) {
        ...
        wasConnected = true;
        FileLog.d("tsupport", String.format(TcpConnection.this + " Connected (%s:%d)", hostAddress, hostPort));
        if (delegate != null) {
            final TcpConnectionDelegate finalDelegate = delegate;
            Utilities.stageQueue.postRunnable(new Runnable() {
                @Override
