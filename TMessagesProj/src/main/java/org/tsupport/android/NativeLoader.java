        ...
        try {
            ...
            try {
                if (Build.CPU_ABI.equalsIgnoreCase("armeabi-v7a")) {
                    ...
                    libSize = sizes[0];
                    libSize2 = sizes[1];
                    FileLog.e("tsupport", "Unsupported arch: " + Build.CPU_ABI);
                }
            } catch (Exception e) {
                FileLog.e("tsupport", e);
                folder = "armeabi";
            ...
            if (destLocalFile != null && destLocalFile.exists()) {
                if (destLocalFile.length() == libSize) {
                    try {
                        FileLog.d("tsupport", "Load local lib");
                        System.load(destLocalFile.getAbsolutePath());
                        nativeLoaded = true;
                        return;
                    } catch (Error e) {
            ...
            FileLog.e("tsupport", "Library not found, arch = " + folder);
