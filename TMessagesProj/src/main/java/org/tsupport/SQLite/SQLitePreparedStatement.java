...
public class SQLitePreparedStatement {
	...
	public SQLitePreparedStatement(SQLiteDatabase db, String sql, boolean cache, boolean finalize) throws SQLiteException {
		finalizeAfterQuery = finalize;
        if (cache)
		    sqliteStatementHandle = prepare(db.getSQLiteHandleCache(), sql);
        else
            sqliteStatementHandle = prepare(db.getSQLiteHandleInternal(), sql);
	}
